...
export default function EditFilling({ filling }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    tenNhanBanh: filling.tenNhanBanh,
    _method: 'PUT',
  });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('nhanbanh.update', filling.id));
  };

  return (
    <AdminLayout>
      <Head title={`Sửa nhân bánh: ${filling.tenNhanBanh}`} />
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900 text-dark">Chỉnh sửa nhân bánh</h1>
            <p className="text-slate-500 text-sm mt-1 font-medium">
               Cập nhật hương vị ID: <span className="text-primary font-black">#{filling.id}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm text-dark space-y-8">
           ...
           <div className="pt-4 border-t border-slate-50 flex items-center gap-3">
              ...
              >
                <Save size={18} />
                {processing ? 'Đang cập nhật...' : 'Cập nhật nhân bánh'}
              </button>
              <Link 
                href={route('nhanbanh.index')}
