...
export default function EditCategory({ danhmuc }: Props) {
  const [preview, setPreview] = React.useState<string | null>(danhmuc.img_url);

  const { data, setData, post, processing, errors } = useForm({
    tendanhmuc: danhmuc.tendanhmuc,
    thutu: danhmuc.thutu,
    anhien: danhmuc.anhien,
    mota: danhmuc.mota || '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('danhmucDM.update', danhmuc.id));
  };

  return (
    <AdminLayout>
      <Head title={`Sửa danh mục: ${danhmuc.tendanhmuc}`} />

      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link 
            href={route('danhmucDM.index')}
            className="p-2 bg-white border border-slate-200 rounded-xl text-slate-500 hover:text-primary transition-all shadow-sm"
          >
            <ArrowLeft size={18} />
          </Link>
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900 text-dark">Chỉnh sửa danh mục</h1>
            <p className="text-slate-500 text-sm mt-1">
               Cập nhật thống tin phần loại sản phẩm.
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm text-dark space-y-6">
               <div className="space-y-2">
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                     <Layers size={14} /> Tên danh mục
                  </label>
                  <input 
                    type="text"
                    value={data.tendanhmuc}
                    onChange={e => setData('tendanhmuc', e.target.value)}
                    className={cn(
                      ...
                      errors.tendanhmuc && "border-red-500"
                    )}
                  />
                  {errors.tendanhmuc && <p className="text-red-500 text-[10px] mt-1 italic font-bold">{errors.tendanhmuc}</p>}
               </div>
               ...
               <div className="space-y-2">
                  ...
                  <input 
                    type="number"
                    value={data.thutu}
                    onChange={e => setData('thutu', Number(e.target.value))}
                    className={cn(
                      "w-full bg-slate-50 border border-slate-100 rounded-2xl py-3 px-5 text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all",
               ...
               <div className="space-y-2">
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                     <AlignLeft size={14} /> Mã tả danh mục
                  </label>
                  <textarea 
                    value={data.mota}
                    onChange={e => setData('mota', e.target.value)}
                    rows={4}
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-3 px-5 text-sm font-medium focus:ring-2 focus:ring-primary/20 outline-none transition-all resize-none"
                  />
               </div>
                <div className="space-y-4">
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                     <ImageIcon size={14} /> Ảnh danh mục
                  </label>
                  ...
                  {preview ? (
                    ...
                    >
                       ...
                       <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Chọn ảnh danh mục</p>
                       <p className="text-[10px] text-slate-300">Tối đa 2MB (JPG, PNG, WebP, AVIF)</p>
                    </div>
                  )}
                  <input 
                    id="img"
          ...
          <div className="md:col-span-1 space-y-6">
            <div className="bg-white p-6 rounded-[28px] border border-slate-200 shadow-sm text-dark sticky top-24">
               <div className="space-y-6">
                  <div>
                    <label className="block text-xs font-bold mb-4 text-slate-500 uppercase tracking-widest">Trạng thái hiện</label>
                    <div className="space-y-3">
                       ...
                       <label className={cn(
                         "flex items-center gap-3 p-3.5 rounded-2xl border-2 transition-all cursor-pointer group",
                         data.anhien === 0 ? "bg-slate-100 border-slate-400" : "bg-slate-50/50 border-slate-100 hover:border-slate-300"
                       )}>
                          <input 
                            type="radio" 
                            name="anhien" 
                  ...
                  <div className="pt-6 border-t border-slate-50 space-y-3">
                     ...
                     >
                       <Save size={18} />
                       {processing ? 'Đang lưu...' : 'Cập nhật danh mục'}
                     </button>
                     <Link 
                       href={route('danhmucDM.index')}
