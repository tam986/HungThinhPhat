...
interface Props {
  danhmuc: PostCategory;
}

export default function EditPostCategory({ danhmuc }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    tendm: danhmuc.tendm,
    thutu: danhmuc.thutu || '',
    anhien: danhmuc.anhien === null ? 1 : danhmuc.anhien,
    _method: 'PUT',
  });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('danhmuc.update', danhmuc.id));
  };

  return (
    <AdminLayout>
      <Head title={`Sửa danh mục: ${danhmuc.tendm}`} />
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1 font-medium">
               Cập nhật ID: <span className="text-primary font-black">#{danhmuc.id}</span>
            </p>
          </div>
        </div>
      </div>
