...
export default function EditPost({ post: initialPost, categories }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    tieude: initialPost.tieude,
    slug: initialPost.slug,
    seotieude: initialPost.seotieude || initialPost.tieude,
    motangan: initialPost.motangan || '',
    noidung: initialPost.noidung,
    id_danhmuc: initialPost.id_danhmuc,
    anhien: initialPost.anhien,
    anhdaidien: null as File | null,
    _method: 'PUT',
  });

  const [preview, setPreview] = React.useState<string | null>(initialPost.anhdaidien_url);
  const [isAutoSlug, setIsAutoSlug] = React.useState(false);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('baiviet.update', initialPost.id), {
      forceFormData: true,
    });
  };
  ...
  return (
    <AdminLayout>
      <Head title={`Sửa bài: ${initialPost.tieude}`} />
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900 italic">Chỉnh sửa bài viết</h1>
            <p className="text-slate-500 text-sm mt-1 font-medium">
               ID: <span className="font-mono text-primary">#{initialPost.id}</span> • Cập nhật nội dung về tối ưu SEO.
            </p>
          </div>
        </div>
      </div>
               ...
               <div className="space-y-6">
                  <div>
                    ...
                    <select 
                      ...
                      onChange={e => setData('id_danhmuc', parseInt(e.target.value))}
                      className={cn(
                    ...
                    <div className="relative group">
                       ...
                       {preview && (
                         <div className="mt-3 text-center">
                            <p className="text-[10px] font-bold text-slate-400 italic">Click vào ảnh để thay đổi</p>
                         </div>
                       )}
                    </div>
                  </div>
