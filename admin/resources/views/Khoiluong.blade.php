@extends('page.layout')
@section('title', 'Danh sách khối lượng')
@section('content')
    <div class="container-fluid p-1">
        <div class="d-flex d-md-none justify-content-between mb-3 animate__animated animate__fadeInDown">
            ...
            <a href="{{ route('khoiluong.create') }}" class="btn btn-primary">
                <i class="bi bi-plus fs-5"></i> <span class="d-none d-md-inline">Khối lượng</span>
            </a>
        </div>
        ...
        <div
            ...
            <div class="container d-flex align-items-center gap-4 mt-4 mb-4 pl-0 pt-4 pb-4">
                <div><i class="bi bi-box text-danger bg-light p-4 rounded fs-4"></i></div>
                <div>
                    <h5 class="text-dark">Quản lý khối lượng</h5>
                    <span class="ml-3 text-muted">
                        Hiện <b class="text-primary">{{ $khoiluongs->count() }}</b> trong
                        @if (request('search') || request('sort'))
                            <b class="text-success">{{ $khoiluongs->total() }}</b> khối lượng được lọc
                        @else
                            tổng <b class="text-success">{{ $khoiluongs->total() }}</b> khối lượng
                        @endif
                    </span>
                </div>
            </div>
            <div class="container d-none d-md-flex justify-content-end">
                <a href="{{ route('khoiluong.create') }}" class="btn btn-primary">
                    <i class="bi bi-plus fs-w-20"></i> Khối lượng
                </a>
            </div>
        </div>
            ...
            <div class="card-body p-3">
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @forelse ($khoiluongs as $kl)
                                <tr>
                                    <td class="text-center d-none d-md-table-cell">{{ $kl->id }}</td>
                                    <td class="flex-column align-items-center text-center">
                                        <div>{{ $kl->khoiluong }}</div>
                                        <small
                                            class="{{ $kl->bienthe_count ? 'text-success' : 'text-secondary' }} d-md-none">
                                            Số biến thể: {{ $kl->bienthe_count ?? 0 }}
                                        </small>
                                    </td>
                                    <td class="text-center d-none d-md-table-cell">{{ $kl->bienthe_count }}</td>
                                    <td class="text-center">
                                        <div class="d-none d-md-flex justify-content-center gap-2">
                                            <a href="{{ route('khoiluong.edit', $kl->id) }}"
                                                class="btn btn-warning btn-sm shadow-sm">
                                                <i class="bi bi-eraser text-white"></i>
                                            </a>
                                            <form action="{{ route('khoiluong.destroy', $kl->id) }}" method="POST"
                                                style="display: inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-danger shadow-sm"
                                                    onclick="return confirm('Xác nhận xóa khối lượng?')">
                                        ...
                                        <div class="d-md-none dropdown">
                                            <button class="btn btn-secondary btn-sm dropdown-toggle shadow-sm"
                                                type="button" id="dropdownMenuButton{{ $kl->id }}"
                                                data-bs-toggle="dropdown" aria-expanded="false">
                                                <i class="bi bi-three-dots-vertical"></i>
                                            </button>
                                            <ul class="dropdown-menu"
                                                aria-labelledby="dropdownMenuButton{{ $kl->id }}">
                                                <li>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center">Không có khối lượng cần tìm.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="d-flex justify-content-center">
                    <nav>
                        <ul class="pagination shadow-sm">
                            @if ($khoiluongs->onFirstPage())
                                <li class="page-item disabled"><span class="page-link">«</span></li>
                            @else
                                <li class="page-item">
                                    <a class="page-link" href="{{ $khoiluongs->url(1) }}" title="Về đầu trang">«</a>
                                </li>
                            @endif
                            @if ($khoiluongs->onFirstPage())
                            ...
                            @else
                                <li class="page-item"><a class="page-link"
                                        href="{{ $khoiluongs->previousPageUrl() }}">Prev</a></li>
                            @endif
                            @foreach ($khoiluongs->links()->elements[0] as $page => $url)
                                @if ($page == $khoiluongs->currentPage())
                                ...
                                @endif
                            @endforeach
                            @if ($khoiluongs->hasMorePages())
                                <li class="page-item"><a class="page-link"
                                        href="{{ $khoiluongs->nextPageUrl() }}">Next</a></li>
                            @else
                                <li class="page-item disabled"><span class="page-link">Next</span></li>
                            @endif
                            @if ($khoiluongs->onLastPage())
                                <li class="page-item disabled"><span class="page-link">»</span></li>
                            @else
                                <li class="page-item">
                                    <a class="page-link" href="{{ $khoiluongs->url($khoiluongs->lastPage()) }}"
