...
@section('content')
    <div class="container-fluid p-3 p-md-3">
        <div
            ...
            <div class="d-flex align-items-center gap-4 mt-4 mb-4">
                ...
                <div>
                    <h4 class="text-dark mb-0 fw-bold">Thêm danh mục</h4>
        ...
        <div class="card shadow-lg border-0 mb-4 animate__animated animate__fadeInUp">
            <div class="card-body p-4">
                <form method="POST" action="{{ route('danhmucDM.store') }}" id="createDanhmucForm">
                    @csrf
                    <div class="row g-4">
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text"
                                    ...
                                    id="tendanhmuc" name="tendanhmuc" value="{{ old('tendanhmuc') }}" required>
                                @error('tendanhmuc')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control form-control-lg shadow-sm @error('mota') is-invalid @enderror" id="mota"
                                    name="mota" rows="3">{{ old('mota') }}</textarea>
                                @error('mota')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control form-control-lg shadow-sm @error('anhien') is-invalid @enderror"
                                    ...
                                    <option value="0" {{ old('anhien', 1) == 0 ? 'selected' : '' }}>Hiện</option>
                                    <option value="1" {{ old('anhien', 1) == 1 ? 'selected' : '' }}>Ẩn</option>
                                </select>
                                @error('anhien')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-end gap-2 mt-4">
                        ...
                        <button type="submit" class="btn btn-primary w-100 w-md-auto shadow-sm">
                            <i class="bi bi-save"></i> Thêm danh mục
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        document.getElementById('createDanhmucForm').addEventListener('submit', function(event) {
            event.preventDefault(); // Ngăn form gửi ngay lập tức

            const thutu = document.getElementById('thutu').value;

            // Gọi AJAX để kiểm tra trùng thutu
            fetch('{{ route('danhmucDM.checkthutu') }}', {
                ...
                .then(data => {
                    if (data.exists) {
                        // Nếu thutu đã tồn tại, hiển thị thông báo xác nhận
                        if (confirm('Thứ tự hiện tại đã có danh mục, xác nhận tạo mới?')) {
                            // Nếu nhấn OK, gửi form
                            event.target.submit();
                        }
                        // Nếu nhấn Cancel, không làm gì cả
                    } else {
                        // Nếu không trùng, gửi form ngay
                        event.target.submit();
                    }
                })
                .catch(error => {
