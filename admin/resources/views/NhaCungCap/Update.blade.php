...
@section('content')
    <div class="container-fluid p-3 p-md-3">
        <div
            ...
            <div class="d-flex align-items-center gap-4 mt-4 mb-4">
                <i class="bi bi-truck text-danger bg-light p-4 rounded fs-4"></i>
                <div>
                    <h4 class="text-dark mb-0 fw-bold">Cập nhật nhà cung cấp</h4>
                    <small class="text-muted">ID: {{ $nhacungcap->id }} - {{ $nhacungcap->tennhacungcap }}</small>
                </div>
            </div>
        </div>
        ...
        <div class="card shadow-lg border-0 mb-4 animate__animated animate__fadeInUp">
            <div class="card-body p-4">
                <form method="POST" action="{{ route('nhacungcap.update', $nhacungcap->id) }}" id="updateNhacungcapForm"
                    ...
                    <div class="row g-4">
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                <label for="tennhacungcap" class="form-label fw-semibold">Tên nhà cung cấp</label>
                                <input type="text"
                                    class="form-control form-control-lg shadow-sm @error('tennhacungcap') is-invalid @enderror"
                                    id="tennhacungcap" name="tennhacungcap"
                                    value="{{ old('tennhacungcap', $nhacungcap->tennhacungcap) }}" required>
                                @error('tennhacungcap')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file"
                                    class="form-control form-control-lg shadow-sm @error('hinhanh') is-invalid @enderror"
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number"
                                    ...
                                    id="thutu" name="thutu" value="{{ old('thutu', $nhacungcap->thutu) }}"
                                    min="0" required>
                                @error('thutu')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control form-control-lg shadow-sm @error('anhien') is-invalid @enderror"
                                    ...
                                    <option value="1" {{ old('anhien', $nhacungcap->anhien) == 1 ? 'selected' : '' }}>
                                        Hiện</option>
                                    <option value="0" {{ old('anhien', $nhacungcap->anhien) == 0 ? 'selected' : '' }}>
                                        Ẩn</option>
                                </select>
                                @error('anhien')
                                    <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-end gap-2 mt-4">
                        <a href="{{ route('nhacungcap.detail', $nhacungcap->id) }}"
                        ...
                        <button type="submit" class="btn btn-primary w-100 w-md-auto shadow-sm">
                            <i class="bi bi-save"></i> Cập nhật nhà cung cấp
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        document.getElementById('updateNhacungcapForm').addEventListener('submit', function(event) {
            event.preventDefault();
            const thutu = document.getElementById('thutu').value;
            const originalThutu = {{ $nhacungcap->thutu ?? 'null' }};

            if (thutu == originalThutu) {
                event.target.submit();
                return;
            }

            fetch('{{ route('nhacungcap.checkthutu') }}', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.exists) {
                        if (confirm('Thứ tự hiện tại đã có nhà cung cấp, xác nhận cập nhật?')) {
                            event.target.submit();
                        }
                    } else {
                        event.target.submit();
