...
@section('content')
    <div class="container-fluid p-3 p-md-3">
        <div
            ...
            <div class="d-flex align-items-center gap-4 mt-4 mb-4">
                ...
                <div>
                    <h4 class="text-dark mb-0 fw-bold">Thêm nhà cung cấp</h4>
                    <small class="text-muted">Tạo nhà cung cấp mới</small>
                </div>
            </div>
        </div>
        ...
        <div class="card shadow-lg border-0 mb-4 animate__animated animate__fadeInUp">
            <div class="card-body p-4">
                <form method="POST" action="{{ route('nhacungcap.store') }}" id="createNhacungcapForm"
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text"
                                    ...
                                    id="tennhacungcap" name="tennhacungcap" value="{{ old('tennhacungcap') }}" required>
                                @error('tennhacungcap')
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number"
                                    ...
                                    id="thutu" name="thutu" value="{{ old('thutu', 0) }}" min="0" required>
                                @error('thutu')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control form-control-lg shadow-sm @error('anhien') is-invalid @enderror"
                                    ...
                                    <option value="1" {{ old('anhien', 1) == 1 ? 'selected' : '' }}>Hiện</option>
                                    <option value="0" {{ old('anhien', 1) == 0 ? 'selected' : '' }}>Ẩn</option>
                                </select>
                                @error('anhien')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-end gap-2 mt-4">
                        ...
                        <button type="submit" class="btn btn-primary w-100 w-md-auto shadow-sm">
                            <i class="bi bi-save"></i> Thêm nhà cung cấp
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        document.getElementById('createNhacungcapForm').addEventListener('submit', function(event) {
            ...
            fetch('{{ route('nhacungcap.checkthutu') }}', {
                ...
                .then(data => {
                    if (data.exists) {
                        if (confirm('Thứ tự hiện tại đã có nhà cung cấp, xác nhận tạo mới?')) {
                            event.target.submit();
                        }
                    } else {
                        event.target.submit();
